	...
	if (man_count>=params->CLSYSID_repeat) {
		...
		case 1:// chirp
			if (float(current_time-t_idstart)/1.0E6f>(params->CLSYSID_tExcite+params->CLSYSID_settime)||float(current_time-t_idstart)/1.0E6f<params->CLSYSID_settime) {
				...
				float phase=2*PI*(params->CLSYSID_f_start+k*0.5*current_time)*current_time;
    			id_step = sin(phase);
			}
			break;	
		default: // error
...
int HL::TECS_Update50Hz(void)
{
	...
	bool in_air_alt_control = ((subs->vstatus.main_state==vehicle_status_s::MAIN_STATE_AUTO_MISSION));// TODO::PX4Merge:is it only running in AUTO_MISSION mode
